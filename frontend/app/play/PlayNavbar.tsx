...
import { TShirt, MusicNote, Smiley, GameController } from '@phosphor-icons/react'
...
import MovementGuideModal from './MovementGuideModal'
...
const PlayNavbar: React.FC<PlayNavbarProps> = ({ username, skin }) => {
  ...
  function sendReaction(reaction: string) {
    ...
    signal.emit('reaction', reaction)
    setShowReactions(false)
  }

  function onClickGuideButton() {
    setModal('Movement Guide') // 👈 mở modal hướng dẫn
  }
  ...
  return (
    <div className='bg-primary w-full h-14 absolute bottom-0 flex flex-row items-center p-2 gap-2 select-none'>
      ...
      {/* Nút Hướng dẫn di chuyển */}
      <button
        title='Hướng dẫn di chuyển'
        className='aspect-square grid place-items-center rounded-lg p-1 outline-none bg-secondary hover:bg-light-secondary animate-colors'
        onClick={onClickGuideButton}
      >
        <GameController className='h-8 w-8 text-black'/>
      </button>
      <MovementGuideModal />
    </div>
  )
